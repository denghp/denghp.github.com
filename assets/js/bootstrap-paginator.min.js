!function ($) {
    ...
    BootstrapPaginator.prototype = {init: function (element, options) {
        this.$element = $(element), this.currentPage = 1, this.lastPage = 1, this.setOptions(options), this.initialized = !0
        ...
        this.$element.off("page-clicked"), this.$element.off("page-changed"), "function" == typeof this.options.onPageClicked && this.$element.bind("page-clicked", this.options.onPageClicked), "function" == typeof this.options.onPageChanged && this.$element.on("page-changed", this.options.onPageChanged)
    }, destroy: function () {
        this.$element.off("page-clicked"), this.$element.off("page-changed"), $.removeData(this.$element, "bootstrapPaginator"), this.$element.empty()
    }, show: function (page) {
        ...
        switch (this.$element.trigger("page-clicked", [event, type, page]), type) {
            case"first":
                this.showFirst();
                break;
            case"prev":
        ...
        return output.first = this.currentPage > 1 ? 1 : null, output.prev = this.currentPage > 1 ? this.currentPage - 1 : null, output.next = this.currentPage < totalPages ? this.currentPage + 1 : null, output.last = this.currentPage < totalPages ? totalPages : null, output.current = this.currentPage, output.total = totalPages, output.numberOfPages = this.options.numberOfPages, output
        ...
        return $(this).each(function (index, item) {
            ...
            if (!data)return $this.data("bootstrapPaginator", data = new BootstrapPaginator(this, options)), void 0;
            if ("string" == typeof option) {
                if (!data[option])throw"Method " + option + " does not exist";
                result = data[option].apply(data, Array.prototype.slice.call(args, 1))
            } else result = data.setOptions(option)
        }), result
    }, $.fn.bootstrapPaginator.defaults = {containerClass: "", size: "normal", alignment: "left", listContainerClass: "", itemContainerClass: function (type, page, current) {
        return page === current ? "active" : ""
    }, itemContentClass: function (type, page, current) {
        return""
    }, currentPage: 1, numberOfPages: 5, totalPages: 1, pageUrl: function (type, page, current) {
        return null
    }, onPageClicked: null, onPageChanged: null, useBootstrapTooltip: !1, shouldShowPage: !0, itemTexts: function (type, page, current) {
        switch (type) {
            case"first":
                return"&lt;&lt;";
            case"prev":
